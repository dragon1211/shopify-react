...
const Content: FC<ContentProps> = ({
  ...
  const steps = [
      ...
      },
      {
        ...
        items: [
            {title: "都度購入", id:PurchaseType.NORMAL, selected: form.purchaseType === PurchaseType.NORMAL, onSelect: () => setValue('purchaseType', PurchaseType.NORMAL), description:"お試しで買ってみたい方にオススメ", small: false },
    ...
    <VStack justifyContent="center" position="relative" zIndex="2"  shadow="xl" bg="white" borderRadius="20px" mt={{base:"48px", md:"90px"}} px={{base:"18px", md:"41.5px"}}>
      ...
      >
            ...
            <Flex pb={{base:"22px"}} justifyContent="center" w={{base:"100%", md:"440px"}}>
              <Box ml={{base:0, md:"-10%"}} w={{base:"100%", md:"60%"}} textAlign="start">
                <Box fontSize={{base:"15px", md:"20px"}}>
                  {form.purchaseType && (form.purchaseType === PurchaseType.SUBSCRIPTION ? "定期購入" : "都度購入")}
          ...
          <PrimaryButton 
            ...
            rightIcon={noChoosingMenu ? <Box mt="-5px"><CartIcon color="white" height="17px"  /></Box> : ''}
...
const Index = () => { 
  ...
  return (
      ...
      >
          <VStack>
            <Box w="100%" maxW={{base:"100%", md:"960px"}} color="#707070">
              <Breadcrumb spacing='8px' separator={<ChevronRightIcon color='gray.500' />} color="#EB6860" position="relative" zIndex={2} pb={{base:"20px", md:"40px"}}>
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink href={`/products/${product.handle}`}>{product.title}</BreadcrumbLink>
                </BreadcrumbItem>

                <BreadcrumbItem isCurrentPage>
                  <BreadcrumbLink href='/pages/choosing-plan'>プラン選択</BreadcrumbLink>
                </BreadcrumbItem>
              </Breadcrumb>
              <Text textAlign="center" fontSize={{base:"25px", md:"40px"}} zIndex={2} position="relative">
                お客様にあったプランを選択
              </Text>
              <Content 
